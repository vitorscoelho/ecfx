...
public strictfp class ControllerCenaResultadosFusteBackupTesteComDoisGraficos extends ControllerCenaPadrao implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        ObservableList listaTipoGrafico = FXCollections.observableArrayList(
                ...
                TipoGrafico.TensaoAtuantexResistente
        );
    ...
    private void setDadosGraficoAtuantexResistente(StackedAreaChart graficoOndeSeraColocado, GraficoTubulao dadosDeGraficoAtuante, GraficoTubulao dadosDeGraficoResistente, TipoGrafico tipoGrafico) {
        NumberAxis eixoY = (NumberAxis) graficoOndeSeraColocado.getXAxis();

        double maximoY = dadosDeGraficoAtuante.getComprimentTubulaoMetros();

        eixoY.setLowerBound(0);
        eixoY.setUpperBound(maximoY);

        XYChart.Series serieResistenteAlternandoSinal = new XYChart.Series();

        for (int i = 0; i < dadosDeGraficoAtuante.getListaValores().getData().size(); i++) {
            double tensaoAtuante = (double) ((XYChart.Data) dadosDeGraficoAtuante.getListaValores().getData().get(i)).getYValue();
            double profundidade = (double) ((XYChart.Data) dadosDeGraficoAtuante.getListaValores().getData().get(i)).getXValue();
            int sinal = 1;
            if (tensaoAtuante < 0) {
                sinal = -1;
            }
            double tensaoResistente = sinal * (Double) ((XYChart.Data) dadosDeGraficoResistente.getListaValores().getData().get(i)).getYValue();
            serieResistenteAlternandoSinal.getData().add(new XYChart.Data(profundidade, tensaoResistente));
        }

        graficoOndeSeraColocado.getData().remove(graficoOndeSeraColocado.getData());
        graficoOndeSeraColocado.getData().add(dadosDeGraficoAtuante.getListaValores());
        graficoOndeSeraColocado.getData().add(serieResistenteAlternandoSinal);

        this.profundidadeTubulao = maximoY;

        if (graficoOndeSeraColocado.equals(this.areaChart1)) {
            this.funcaoGrafico1 = dadosDeGraficoAtuante.getFuncao();
            this.funcaoGrafico1 = dadosDeGraficoResistente.getFuncao();
            this.tipoGrafico1 = tipoGrafico;
        } else if (graficoOndeSeraColocado.equals(this.areaChart2)) {
            this.funcaoGrafico2 = dadosDeGraficoAtuante.getFuncao();
            this.funcaoGrafico2 = dadosDeGraficoResistente.getFuncao();
            this.tipoGrafico2 = tipoGrafico;
        } else if (graficoOndeSeraColocado.equals(this.areaChart3)) {
            this.funcaoGrafico3 = dadosDeGraficoAtuante.getFuncao();
            this.funcaoGrafico3 = dadosDeGraficoResistente.getFuncao();
            this.tipoGrafico3 = tipoGrafico;
        } else if (graficoOndeSeraColocado.equals(this.areaChart4)) {
            this.funcaoGrafico4 = dadosDeGraficoAtuante.getFuncao();
            this.funcaoGrafico4 = dadosDeGraficoResistente.getFuncao();
            this.tipoGrafico4 = tipoGrafico;
        }
    }
        ...
        ChangeListener listener = (ChangeListener) (ObservableValue observable, Object oldValue, Object newValue) -> {
            ...
            if (!newValue.equals(oldValue)) {
                ...
                if (tipoGrafico.equals(TipoGrafico.CoeficienteDeReacao)) {
                    ...
                    setDados(grafico, graficoTubulao, tipoGrafico);
                    labelMaximos.setText(tipoGrafico.getMaximoOuMinimo() + ": " + " " + arredondar(graficoTubulao.getValorMaximo(), 2) + tipoGrafico.getUnidade() + " em " + arredondar(graficoTubulao.getProfundidadeValorMaximo() / 100, 2) + "m");
                } else if (tipoGrafico.equals(TipoGrafico.TensaoAtuantexResistente)) {
                    GraficoTubulao graficoTubulao1 = Principal.getResultadosAnalise().getGraficoTensao();
                    GraficoTubulao graficoTubulao2 = Principal.getResultadosAnalise().getGraficoTensaoHorizontalResistente();

                    setDadosGraficoAtuantexResistente(grafico, graficoTubulao1, graficoTubulao2, tipoGrafico);
                    labelMaximos.setText(tipoGrafico.getMaximoOuMinimo() + ": " + " " + arredondar(graficoTubulao1.getValorMaximo(), 2) + tipoGrafico.getUnidade() + " em " + arredondar(graficoTubulao1.getProfundidadeValorMaximo() / 100, 2) + "m");
    ...
    private enum TipoGrafico {
        ...
        TensaoAtuantexResistente("Tensão atuante x resistente (kN/m²)", "Máxima", "kN/m²", "Descrição:\r\nTensão horizontal admissível  do solo em kN/m²."),
